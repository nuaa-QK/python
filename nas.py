...
def _subproc_eva(params, eva, gpuq,net_pool):
	ngpu = gpuq.get()
	...
	if MAIN_CONFIG['eva_debug']:
		...
		score = eva.evaluate(item, is_bestNN=blk_wnr,update_pre_weight=ft_sign)
	gpuq.put(ngpu)
	time_cost = time.time() - start_time

	# NAS_LOG << ('eva_ing', len(Network.pre_block)+1, rd, nn_id,
	#             pl_len, spl_id, bt_nm, score, time_cost, os.getpid())
	return score, time_cost, nn_id, spl_id ,net_pool
...
def _do_task(pool, cmnct, eva,net_pool):
	...
	while True:
		while not cmnct.task.empty():
			try:
				...
				item, rd, nn_id, pl_len, spl_id, bt_nm, blk_wnr, ft_sign,net = task_params
				NAS_LOG << ('eva_pre', len(Network.pre_block) + 1, rd, nn_id,
							pl_len, spl_id, bt_nm)
			except:
				break
			if MAIN_CONFIG['subp_eva_debug']:
				result = _subproc_eva(task_params, eva, cmnct.idle_gpuq,net_pool)
			else:
				if blk_wnr ==True:
					net_pool=net
					result = pool.apply_async(
						_subproc_eva,
						(task_params, eva, cmnct.idle_gpuq,net_pool),callback=cmnct.wake_up_train_winner)
				else:
					result = pool.apply_async(
						_subproc_eva,
						(task_params, eva, cmnct.idle_gpuq,net_pool))
...
def _arrange_result(cmnct, net_pl):
	while not cmnct.result.empty():
		r_ = cmnct.result.get()
		if MAIN_CONFIG['subp_eva_debug']:
			score, time_cost, nn_id, spl_id,np = r_
		else:
			score, time_cost, nn_id, spl_id,np = r_.get()
...
def _gpu_batch_task_inqueue(para):
	...
	:return:
	"""
	nn, com, round, nn_id, pool_len, batch_num, block_winner, finetune_sign,net_pool= para
	for spl_id in range(1, batch_num + 1):
		item = nn.item_list[-spl_id]
		task_param = [
			item, round, nn_id, pool_len, spl_id, batch_num, block_winner, finetune_sign,net_pool
		]
...
def _assign_task(net_pool, com, round, batch_num=MAIN_CONFIG['spl_network_round'], block_winner=False):
	...
	for nn, nn_id in zip(net_pool, range(1, pool_len+1)):
		...
		para = nn, com, round, nn_id, pool_len, \
			batch_num, block_winner, finetune_sign,net_pool
...
def _game(eva, net_pool, com, round, process_pool):
	...
	_do_task(process_pool, com, eva,net_pool)
...
def _global_train(net_pl, com, pro_pl, eva_winner):
	for i in range(MAIN_CONFIG['num_opt_best'] // MAIN_CONFIG['num_gpu'] + 1):
		...
		if task_num:
			...
			_do_task(pro_pl, com, eva_winner,net_pl)
